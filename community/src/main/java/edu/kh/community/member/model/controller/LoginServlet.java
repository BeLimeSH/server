...
public class LoginServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// * 모든 doPost() 메서드에 인코딩 변경 코드를 작성해야 함
		...
		try {
			...
			if(loginMember != null) { //성공
				 ...
				 *    
				 * 2) 응답에 담신 쿠키가 클라이언트에 파일 형태로 저장
				 * 
				 * 3) 이후 특정 주소 요청 시 쿠키 파일을 브라우저가 찾아 
				 *    자동으로 요청에 실어서 보냄.
				 * 
...
//				Cookie c = new Cookie("클라이언트 쪽에 저장될 쿠키 이름", "쿠키 내용");
			...
			// 클라이언트 로그인 요청 -> 서버 요청처리(Servlet) -> JSP위임
			
			// 1. forward(요청 위임)
			// - Servlet으로 응답 화면 만들기가 불편하기 때문에 
			//	 JSP로 req,resp 객체를 위임하여 요청에 대한 응답화면을 대신 만듦.
			//	 *** 화면이 변경되어도 요청 주소가 유지된다. ***
			
			// 2. Redirect(재요청)
			// - 현재 Servlet에서 응답 페이지를 만들지 않고
			//	 응답 페이지를 만들 수 있는 다른 요청의 주소로 클라이언트를 이동 시킴
			
			// - 클라이언트 요청 
			// -> 기존 HttpServletRequest/Response 제거
			// -> 새로운 HttpServletRequest/Response 생성
			
			// --> 리다이렉트 시 request 객체가 유지 X
			//	   특정 데이터를 전달하거나 유지하고 싶으면
			//	   session 또는 application 범위에 세팅해야 함.
