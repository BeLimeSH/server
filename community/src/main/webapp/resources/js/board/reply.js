...
addReply.addEventListener("click", function(){ //댓글 등록 버튼이 클릭되어 있을 때
...
function showUpdateReply(replyNo, btn){
    ...
    if(temp.length>0){ //수정이 한 개 이상 열린 경우
        ...
        if(confirm("다른 댓글이 수정중입니다. 현재 댓글을 수정 하시겠습니까?")){ //확인
    ...
    //2. 행 내용 삭제 전 현재 상태 저장(백업)
    //   (전역변수 이용)
    beforeReplyRow = replyRow.innerHTML;

    //3. 댓글에 작성되어 있던 내용만 얻어오기 -> 새롭게 생성된 textarea에 추기될 예정
    // replyRow.children[1].innerHTML //br 태그 유지
    ...
    //8. 버튼 영역에 버튼 추가 후 replyRow(행)에 버튼 영역 추가
