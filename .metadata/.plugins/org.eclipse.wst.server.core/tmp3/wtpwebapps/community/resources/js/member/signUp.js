...
memberTel.addEventListener("input", function(){
    ...
    if(memberTel.value.length == 0){
        ...
        checkObj.memberTel = false; //유효하지 않은 상태 기록
        return;
    }
    
    //전화번호 정규식
    const regExp = /^0(1[016789]|2|[3-6][1-5]|70)\d{3,4}\d{4}$/;
    
    // 010/011/016/017/019
    // 02/031/032/041/042/051/052/061/062/063/064
    // 070
    ...
    if(regExp.test(memberTel.value)){ //유효한 경우
        ...
        checkObj.memberTel = true; //유효한 상태 기록
    } else { //유효하지 않은 경우
...
memberEmail.addEventListener("input", function(){
    ...
    if(memberEmail.value.length == 0){
        ...
        checkObj.memberEmail = false; //유효하지 않은 상태 기록
        return;
    }
    ...
    if(regExp.test(memberEmail.value)){
        ...
        $.ajax({
            url : "emailDupCheck",      //필수 속성 url
                                        //상대경로 작성
            data : { "memberEmail" : memberEmail.value }, //data 속성 : 비동기 통신 시 서버로 전달한 값을 작성(JS 객체 형식)
                                                          //-> 비동기 통신 시 input에 입력된 값을 "memberEmail"이라는 key 값(파라미터)으로 전달
            type : "GET", //데이터 전달 방식
            success : function(result){
                ...
                if(result == 1){ //중복 O
                    ...
                    checkObj.memberEmail = true; //유효한 상태 기록
...
memberNickname.addEventListener("input", function(){
    ...
    if(memberNickname.value.length == 0){
        nicknameMessage.innerText = "영어/숫자/한글 2~10 글자 사이로 작성해주세요.";
        nicknameMessage.classList.remove("confirm", "error");

        checkObj.memberNickname = false; //유효하지 않은 상태 기록
        return;
    }
        ...
        $.ajax({
            ...
            success : function(res){
                ...
                if(res == 0){ //닉네임 중복 X
                    ...
                    checkObj.memberNickname = true; //유효한 상태 기록
...
memberPw.addEventListener("input", function(){
    ...
    if(memberPw.value.length == 0){
        ...
        checkObj.memberPw = false; //유효하지 않은 상태 기록
        return;
    }
    ...
    if(regExp.test(memberPw.value)){ //비밀번호 유효
        ...
        checkObj.memberPw = true; //유효한 상태 기록

        if(memberPwConfirm.value.length==0){ //확인작성X
...
// memberPwConfirm.addEventListener("input", 함수정의O 함수호출X);
//  -> 이벤트가 발생하였을 때 정의된 함수를 호출한다.
...
function checkPw(){ //비밀번호 일치 검사
    ...
    if(memberPw.value == memberPwConfirm.value){
        ...
        checkObj.memberPwConfirm = true; //유효한 상태 기록
    } else {
        pwMessage.innerText = "비밀번호가 일치하지 않습니다.";
        pwMessage.classList.add("error");
        pwMessage.classList.remove("confirm");

        checkObj.memberPwConfirm = false; //유효하지 않은 상태 기록
    }
}
