...
document.getElementById("select1").addEventListener("click", function(){
    ...
    $.ajax({
        ...
        dataType : "JSON", //dataType : 응답데이터 형식을 지정
                           //-> "JSON"으로 지정시 자동으로 JS객체로 변환 -> JSON.parse 안써도 됨!

        success : function(member){
            console.log(member); //JS 객체 형태 문자열
                                 //단, dataType : "JSON" 추가 후에는 JS 객체로 변환
            ...
            if(member != null){ //회원정보 존재
                ...
                li5.innerText = "가입 일 : " + member.enrollDate;
...
function selectAll(){ //회원 전체 조회 함수
    ...
    $.ajax({
        ...
        dataType : "JSON",          //응답 데이터의 형식을 "json"으로 지정 -> 자동으로 JS 객체로 변환됨
...
(function(){
    ...
    window.setInterval(selectAll, 30000); //30초
    // 함수 이름만 작성 -> 함수 코드가 대입
    // -> 30초마다 selectAll 함수 수행
