...
public class ServletEx2 extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// -> getParameter()는 전달 된 input 태그의 name이 하나일 때만 가능
		// --> 같은 name이 여러 개면 String[]으로 반환하는
		...
		// ** 스트림을 통해서 그냥 문자열을 내보내면 정상 출력 X **
		// 왜? 전달되는 응답 데이터가 어떤 형식인지, 
		//	   문자 인코딩은 어떤 것인지 지정해주지 않았기 때문.
		...
		//HTML 코드를 자바(Servlet)에서 작성하여 클라이언트와 연결된
		//응답 출력용 스트림(out)을 이용해 출력
		...
		if(coffee != null) {
			...
			out.println("<li>주문한 메뉴가 없습니다.</li>");
		}
